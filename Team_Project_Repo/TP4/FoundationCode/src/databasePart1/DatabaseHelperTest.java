 ...
 * <p>
 * This class contains unit tests for verifying the functionality of the DatabaseHelper class,
 * including database operations for reviews, trusted reviewers, questions, and answers.
 * </p>
 * 
 * @author CSE360 Team
 * @version 1.0
...
public class DatabaseHelperTest {
    /** The database helper instance used for testing. */
    private DatabaseHelper dbHelper;
    
    /** A test reviewer username used for creating test data. */
    private String testReviewer = "testReviewer";
    
    /** A test student username used for creating test data. */
    private String testStudent = "testStudent";
    
    /** A unique ID for test items to avoid conflicts between test runs. */
    private String testAssociatedId;
    
    /**
     * Sets up the test environment before each test.
     * <p>
     * Creates a new DatabaseHelper instance and establishes a database connection.
     * Generates a unique ID for each test run to avoid conflicts between tests.
     * </p>
     * 
    ...
    /**
     * Tests adding and retrieving a review from the database.
     * <p>
     * Verifies that a review can be saved to the database and then retrieved
    ...
    /**
     * Tests updating a review in the database.
     * <p>
     * Verifies that a review's content can be updated in the database.
     * </p>
     * 
     * @throws SQLException if a database access error occurs
     */
    ...
    /**
     * Tests deleting a review from the database.
     * <p>
     * Verifies that a review can be deleted from the database.
     * </p>
     * 
     * @throws SQLException if a database access error occurs
     */
    ...
    /**
     * Tests retrieving reviews by reviewer from the database.
     * <p>
     * Verifies that reviews can be retrieved based on the reviewer's username.
     * </p>
     * 
     * @throws SQLException if a database access error occurs
     */
    @Test
    public void testGetReviewsByReviewer() throws SQLException {
        // Create and add reviews by different reviewers
        Review review1 = new Review(testReviewer, "Review 1", UUID.randomUUID().toString());
        Review review2 = new Review(testReviewer, "Review 2", UUID.randomUUID().toString());
        Review review3 = new Review("otherReviewer", "Review 3", UUID.randomUUID().toString());
        
        dbHelper.addReview(review1);
        dbHelper.addReview(review2);
        dbHelper.addReview(review3);
        
        // Get reviews by testReviewer
        List<Review> reviews = dbHelper.getReviewsByReviewer(testReviewer);
        ...
        for (Review r : reviews) {
            if (r.getId().equals(review1.getId())) foundReview1 = true;
            if (r.getId().equals(review2.getId())) foundReview2 = true;
        }
        
        assertTrue("Should find review1", foundReview1);
        assertTrue("Should find review2", foundReview2);
        
        // Clean up
        dbHelper.deleteReview(review1.getId());
        dbHelper.deleteReview(review2.getId());
        dbHelper.deleteReview(review3.getId());
    }
    
    /**
     * Tests the trusted reviewers functionality in the database.
     * <p>
     * Verifies that trusted reviewers can be added, updated, and retrieved.
     * </p>
     * 
     * @throws SQLException if a database access error occurs
     */
    ...
    /**
     * Tests retrieving reviews from trusted reviewers.
     * <p>
     * Verifies that reviews can be filtered based on trusted reviewer status.
     * </p>
     * 
     * @throws SQLException if a database access error occurs
     */
    ...
    /**
     * Tests loading all discussion content from the database.
     * <p>
     * Verifies that questions and answers can be loaded from the database into
    ...
    /**
     * Tests the fixReviewsTable method.
     * <p>
     * Verifies that the method runs without throwing exceptions.
     * </p>
     * 
     * @throws SQLException if a database access error occurs
     */
    ...
    /**
     * Tests review rating operations in the database.
     * <p>
     * Verifies that reviews with different ratings can be stored and retrieved,
     * and that aggregate operations on ratings (average, min, max) work correctly.
     * </p>
     * 
     * @throws SQLException if a database access error occurs
     */
